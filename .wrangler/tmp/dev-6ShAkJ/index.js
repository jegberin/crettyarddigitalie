...
// .wrangler/tmp/bundle-Mi3e0h/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-Mi3e0h/middleware-insertion-facade.js
init_modules_watch_stub();
...
__name(buildContactNotification, "buildContactNotification");
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
      ...
      <tr><td style="padding:32px 40px;">
        ...
        <p style="margin:0;font-size:13px;"><a href="mailto:info@crettyarddigital.ie" style="color:#12B388;">info@crettyarddigital.ie</a> &nbsp;\xB7&nbsp; <a href="https://crettyarddigital.ie" style="color:#12B388;">crettyarddigital.ie</a></p>
      </td></tr>
      <tr><td style="padding:16px 40px;background:#f3f4f5;border-top:1px solid #e5e7eb;">
...
__name(buildThankYou, "buildThankYou");
function buildQuoteNotification(d) {
  const fmt = /* @__PURE__ */ __name((n) => `\u20AC${n.toLocaleString("en-IE")}`, "fmt");
  const fmtRange = /* @__PURE__ */ __name((low, high) => {
    const l = low ?? 0;
    const h = high ?? 0;
    if (l === 0 && h === 0) return "\u2014";
  ...
  const subAnswerRows = Object.entries(d.subAnswers).filter(([, v]) => v !== void 0 && v !== "").map(
    ([k, v]) => `<tr><td style="padding:5px 0;color:#333;font-size:14px;width:45%;font-weight:bold;">${esc(subAnswerLabels[k] ?? k)}</td><td style="padding:5px 0;color:#555;font-size:14px;">${esc(v ?? "")}</td></tr>`
  ).join("");
  const serviceRows = d.serviceLabels.map(
    (s) => `<tr><td style="padding:4px 0;color:#555;font-size:14px;">\u2713 ${esc(s)}</td></tr>`
  ).join("");
  const lineRows = d.ballpark.lines.map((l) => {
    const oneOff = (l.oneOffLow ?? 0) > 0 || (l.oneOffHigh ?? 0) > 0 ? fmtRange(l.oneOffLow, l.oneOffHigh) : "";
    const monthly = (l.monthlyLow ?? 0) > 0 || (l.monthlyHigh ?? 0) > 0 ? `${fmtRange(l.monthlyLow, l.monthlyHigh)}/mo` : "";
    const tags = [
    ...
    const priceCell = [oneOff, monthly].filter(Boolean).join(" \xB7 ");
    return `<tr>
...
__name(buildQuoteNotification, "buildQuoteNotification");
...
var VALID_TIMING = /* @__PURE__ */ new Set(["asap", "this-quarter", "next-quarter", "exploring"]);
var VALID_CONTACT_METHODS = /* @__PURE__ */ new Set(["email", "phone", "whatsapp"]);
...
async function handleQuote(request, env) {
  ...
  const services = Array.isArray(body.services) ? body.services.filter(
    (s) => typeof s === "string" && VALID_SERVICES.has(s)
  ) : [];
  ...
  const serviceLabels = Array.isArray(body.serviceLabels) ? body.serviceLabels.filter((s) => typeof s === "string").slice(0, 20) : services;
  ...
  const subAnswersRaw = body.subAnswers ?? {};
  const subAnswers = {};
  ...
  const ballparkIn = body.ballpark ?? {};
  ...
  const safeLines = linesIn.slice(0, 40).map((raw) => {
    ...
    return {
      ...
      note: l.note ? String(l.note).slice(0, 400) : void 0,
      oneOffLow: typeof l.oneOffLow === "number" ? Math.max(0, Math.min(l.oneOffLow, 1e6)) : void 0,
      oneOffHigh: typeof l.oneOffHigh === "number" ? Math.max(0, Math.min(l.oneOffHigh, 1e6)) : void 0,
      monthlyLow: typeof l.monthlyLow === "number" ? Math.max(0, Math.min(l.monthlyLow, 1e5)) : void 0,
      monthlyHigh: typeof l.monthlyHigh === "number" ? Math.max(0, Math.min(l.monthlyHigh, 1e5)) : void 0,
  ...
  const ballpark = {
    lines: safeLines,
    oneOffLow: typeof ballparkIn.oneOffLow === "number" ? Math.max(0, Math.min(ballparkIn.oneOffLow, 1e7)) : 0,
    oneOffHigh: typeof ballparkIn.oneOffHigh === "number" ? Math.max(0, Math.min(ballparkIn.oneOffHigh, 1e7)) : 0,
    monthlyLow: typeof ballparkIn.monthlyLow === "number" ? Math.max(0, Math.min(ballparkIn.monthlyLow, 1e6)) : 0,
    monthlyHigh: typeof ballparkIn.monthlyHigh === "number" ? Math.max(0, Math.min(ballparkIn.monthlyHigh, 1e6)) : 0,
    hasTalkOnly: ballparkIn.hasTalkOnly === true || services.length === 1 && services[0] === "not-sure"
  };
  const businessNameRequired = !ballpark.hasTalkOnly && ballpark.lines.length > 0 && ballpark.oneOffHigh > 1e3;
  ...
  try {
    ...
    const intro = ballpark.hasTalkOnly ? "Thanks for getting in touch. I\u2019ve received your request and I\u2019ll reach out within one business day to set up a no-obligation chat about what you need." : "Thanks for your request. I\u2019ve received everything you told me, along with the ballpark estimate you saw on screen. I\u2019ll review it properly and come back to you within one business day with a tailored fixed quote.";
...
__name(withLinkHeaders, "withLinkHeaders");
var worker_default = {
