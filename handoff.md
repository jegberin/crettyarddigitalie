...
- **Post-deploy auto-refresh:** the worker also has a `version_metadata` binding (`CF_VERSION_METADATA`). On every fetch, it compares the current deploy's UUID to the value stored in KV under `__build_version`. If they differ, the worker stamps the new version into KV and kicks a full refresh in the background via `ctx.waitUntil`. The KV stamp happens *before* the refresh runs, so a thundering herd of post-deploy requests doesn't trigger multiple parallel refreshes. Net effect: every Workers Builds deploy automatically gets a fresh markdown cache without waiting for the next 03:00 UTC cron.
